...
main(input) {
  ...
  // 多 agent 是否串行或并行由依赖关系决定；routePlan 必须先收敛 gamedesign 路线，再合并 git、quality 与 bootstrap 等支撑路线；
  // 最终输出前必须完成统一汇总。
  ...
  var supportRoutePlan = decideSupportRoutes(milestoneResult, gamedesignRoutePlan, aiUserAgentsContext)
  // routePlan 不是一次性直接生成，而是必须先得到 gamedesignRoutePlan，
  // 再合并 git、quality、bootstrap 等支撑路线。
  // 若存在用户工程根目录下的 `/AI-User/agents`，合并 route 时必须一并吸收其可用 agent 上下文。
  var routePlan = mergeRoutePlans(gamedesignRoutePlan, supportRoutePlan, aiUserAgentsContext)
  var results = []
